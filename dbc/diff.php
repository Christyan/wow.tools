    ...
    <?php if (!empty($currentDB)) { ?>
        <form class='form-inline' id='dbcform' action='/dbc/diff.php' method='GET'>
            ...
            <select id='newbuild' name='new' class='form-control form-control-sm buildFilter'>
                <?php
                foreach ($versions as $row) {?>
                    <option value='<?=$row['version']?>'<?php if (!empty($_GET['new']) && $row['version'] == $_GET['new']) {
                        echo " SELECTED";
                                   }?>><?=$row['version']?></option>
                    <?php
                }
            ...
            <div class="form-check" style='padding-left: 5px;'>
                <input type="checkbox" class="form-check-input" id="useHotfixes" name="useHotfixes" <?php if (!empty($_GET['useHotfixes'])) {
                    ?>CHECKED<?php
                                                                                                    } ?>>
