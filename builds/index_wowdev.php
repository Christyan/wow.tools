...
if (!empty($_GET['api']) && $_GET['api'] == "buildinfo") {
    ...
    wowdev_versions.id as versionid,
    wowdev_versions.cdnconfig,
    wowdev_versions.buildconfig,
    wowdev_versions.patchconfig,
    wowdev_versions.complete as versioncomplete,
    wowdev_versions.product as versionproduct,
    wowdev_buildconfig.id as buildconfigid,
    wowdev_buildconfig.description,
    wowdev_buildconfig.product,
    wowdev_buildconfig.encoding,
    wowdev_buildconfig.encoding_cdn,
    wowdev_buildconfig.root,
    wowdev_buildconfig.root_cdn,
    wowdev_buildconfig.install,
    wowdev_buildconfig.install_cdn,
    wowdev_buildconfig.download,
    wowdev_buildconfig.download_cdn,
    wowdev_buildconfig.size,
    wowdev_buildconfig.size_cdn,
    wowdev_buildconfig.unarchivedcount,
    wowdev_buildconfig.unarchivedcomplete,
    wowdev_buildconfig.complete as buildconfigcomplete,
    wowdev_buildconfig.builton,
    wowdev_cdnconfig.archivecount,
    wowdev_cdnconfig.archivecomplete,
    wowdev_cdnconfig.indexcomplete,
    wowdev_cdnconfig.patcharchivecount,
    wowdev_cdnconfig.patcharchivecomplete,
    wowdev_cdnconfig.patchindexcomplete,
    wowdev_cdnconfig.complete as cdnconfigcomplete,
    wowdev_patchconfig.patch,
    wowdev_patchconfig.complete as patchconfigcomplete
    FROM wowdev_versions
    LEFT OUTER JOIN wowdev_buildconfig ON wowdev_versions.buildconfig=wowdev_buildconfig.hash
    LEFT OUTER JOIN wowdev_cdnconfig ON wowdev_versions.cdnconfig=wowdev_cdnconfig.hash
    LEFT OUTER JOIN wowdev_patchconfig ON wowdev_versions.patchconfig=wowdev_patchconfig.hash
    WHERE wowdev_versions.id = ?
    ");
    ...
    if (!empty($build['builton'])) {
        echo "<tr><td>Compiled at</td><td>" . $build['builton'] . "</td></tr>";
    }
    echo "</table>";
    echo "<h4>Configs</h4>";
    ...
    if (!empty($build['buildconfig'])) {
        echo "<tr><td>Build config (<a href='#' data-toggle='modal' data-target='#configModal' onClick='fillConfigModal(\"" . $build['buildconfig'] . "\", \"wowdev\")'>show</a>)</td><td>";
    ...
    if (!empty($build['cdnconfig'])) {
        echo "<tr><td>CDN config (<a href='#' data-toggle='modal' data-target='#configModal' onClick='fillConfigModal(\"" . $build['cdnconfig'] . "\", \"wowdev\")'>show</a>)</td><td>";
    ...
    if (!empty($build['patchconfig'])) {
        echo "<tr><td>Patch config (<a href='#' data-toggle='modal' data-target='#configModal' onClick='fillConfigModal(\"" . $build['patchconfig'] . "\", \"wowdev\")'>show</a>)</td><td>";
    ...
    if (!empty($build['encoding'])) {
        ...
        if (!empty($build['encoding_cdn']) && doesFileExist("data", $build['encoding_cdn'], $allowedproducts["wowdev"]['cdndir'])) {
    ...
    if (!empty($build['root'])) {
        ...
        if (!empty($build['root_cdn']) && doesFileExist("data", $build['root_cdn'], $allowedproducts["wowdev"]['cdndir'])) {
    ...
    if (!empty($build['install'])) {
        ...
        if (!empty($build['install_cdn']) && doesFileExist("data", $build['install_cdn'], $allowedproducts["wowdev"]['cdndir'])) {
    ...
    if (!empty($build['download'])) {
        ...
        if (!empty($build['download_cdn']) && doesFileExist("data", $build['download_cdn'], $allowedproducts["wowdev"]['cdndir'])) {
    ...
    if (!empty($build['size'])) {
        ...
        if (!empty($build['size_cdn']) && doesFileExist("data", $build['size_cdn'], $allowedproducts["wowdev"]['cdndir'])) {
    ...
    if (!empty($_GET['config']) && strlen($_GET['config']) == 32 && ctype_xdigit($_GET['config'])) {
        echo "<pre>";
        echo file_get_contents("/tmp/wowdevcache/" . $_GET['config']);
        echo "</pre>";
    } else {
        die("Invalid config!");
    }
...
wowdev_versions.buildconfig,
wowdev_versions.patchconfig,
wowdev_versions.releasetime as releasetime,
wowdev_versions.complete as versioncomplete,
wowdev_versions.product as versionproduct,
wowdev_buildconfig.id as buildconfigid,
wowdev_buildconfig.description,
...
LEFT OUTER JOIN wowdev_cdnconfig ON wowdev_versions.cdnconfig=wowdev_cdnconfig.hash
LEFT OUTER JOIN wowdev_patchconfig ON wowdev_versions.patchconfig=wowdev_patchconfig.hash
ORDER BY wowdev_buildconfig.description DESC
";
$res = $pdo->query($query);
...
<div class='container-fluid'>
    ...
    <table id='buildtable' class='table table-sm table-hover maintable' style='clear: both'>
        ...
        <?php foreach ($allbuilds as $row) {
            ...
            echo "<td style='width: 70px'>" . $buildarr['build'];
            ...
            echo "<span class='hash buildconfighash' data-tpr='wowdev'>" . $row['buildconfig'] . "</span>";
            ...
            echo "<a href='#' data-toggle='modal' data-target='#moreInfoModal' onClick='fillVersionModal(" . $row['versionid'] . ", \"wowdev\")'>Show details</a>";
            echo "</td>";
            echo "</tr>\n";
        } ?>
    </table>
