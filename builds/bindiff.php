...
<div class="container-fluid">
       ...
       <div class='input-group'>
          <select class='form-control' name='oldbuild'>
                <?php foreach ($arr as $row) {
                    ?>
                <option value='<?=$row['buildconfig']?>'<?php if (!empty($_GET['oldbuild']) && $row['buildconfig'] == $_GET['oldbuild']) {
                    echo " SELECTED";
         ...
         <select class='form-control' name='newbuild'>
                <?php foreach ($arr as $row) {
                    ?>
                <option value='<?=$row['buildconfig']?>'<?php if (!empty($_GET['newbuild']) && $row['buildconfig'] == $_GET['newbuild']) {
                    echo " SELECTED";
                               }?>><?=$row['description']?></option>
                    <?php
                }?>
          ...
          <select class='form-control' name='oldfile'>
            ...
            if (!empty($_GET['oldbuild'])) {
                ...
                foreach ($oldlines as $line) {
                    ...
                    $filename = $split1[0];
                    if (substr($filename, -4) != ".exe" && substr($filename, -4) != ".dll" && substr($filename, -17) != "World of Warcraft" && substr($filename, -11) != "MacOS\Agent") {
                        continue;
                    }
                    ...
                    if (!empty($_GET['oldfile']) && $filename == $_GET['oldfile']) {
                        echo " SELECTED";
                    }
         ...
         <select class='form-control' name='newfile'>
            ...
            if (!empty($_GET['newbuild'])) {
                ...
                foreach ($newlines as $line) {
                    ...
                    $md5 = str_replace("md5: ", "", $split2[1]);
                    echo "<option value='" . $filename . "' ";
                    if (!empty($_GET['newfile']) && $filename == $_GET['newfile']) {
                        echo " SELECTED";
                    }
                    echo ">" . $filename . "</option>";
                }
 ...
 <input type='submit' class='form-control btn btn-primary' style='margin-left: 10px;'>
    ...
    if (!empty($_GET['oldbuild']) && !empty($_GET['newbuild'])) {
        ...
        echo "<pre>";
        foreach (explode("\n", $diff) as $line) {
            if (substr($line, 0, 3) == "@@ " || substr($line, 0, 3) == "+++" || substr($line, 0, 3) == "---" || substr($line, 0, 3) == "dif") {
                continue;
            }
