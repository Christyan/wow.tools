...
function updateBuildConfig($product)
{
    ...
    foreach ($bcs as $build) {
        if (empty($build['build-uid'])) {
            $build['build-uid'] = "UNKNOWN";
        }
        if (empty($build['build-name'])) {
        ...
        if (!$existingBuild) {
            if ($product == "catalogs") {
                $build['build-name'] = str_replace($build['root'] . "-", "", $build['build-name']);
            }
...
function updateBuildConfigLong($product)
{
    ...
    while ($row = $res->fetch()) {
        ...
        if (!doesFileExist("config", $row['cdnconfig'], $allowedproducts[$product]['cdndir'])) {
            continue;
        }
