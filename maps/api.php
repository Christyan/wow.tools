...
if ($_GET['type'] == "areaname") {
    ...
    foreach ($pathcsv as $path) {
        if (empty($path['FromTaxiNode']) || empty($path['ToTaxiNode'])) {
            continue;
        }
    ...
    foreach ($csv as $entry) {
        if (!isset($entry['ContinentID']) || $entry['ContinentID'] != $mapid) {
            continue;
        }
        if (empty($entry['Pos[0]']) && empty($entry['Pos[1]'])) {
            continue;
        }
        if (!isset($entry['MountCreatureID[0]'])) {
            continue;
        }
