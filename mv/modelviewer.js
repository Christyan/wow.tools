    ...
    for (const result of results){
        ...
        if (result.ResultType == "creature"){
            ...
            if (result['ModelMaterialResourcesID[0]'] != "0" && !materialResourceMap.has(result['ModelMaterialResourcesID[0]'])){
                const materialResponse = await fetch(API_URL + "/api/peek/TextureFileData/?build=" + Current.buildName + "&col=MaterialResourcesID&val=" + result['ModelMaterialResourcesID[0]']);
...
async function findCreatureModelDataRows(){
    const response = await fetch(API_URL + "/api/find/CreatureModelData/?build=" + Current.buildName + "&col=FileDataID&val=" + Current.fileDataID);
...
async function loadCreatureDisplays(cmdRows){
    ...
    for (const cmdRow of cmdRows) {
        cdiPromises[index++] = fetch(API_URL + "/api/find/CreatureDisplayInfo/?build=" + Current.buildName + "&col=ModelID&val=" + cmdRow.ID);
    }
...
async function loadItemDisplays(){
    const response = await fetch(API_URL + "/api/peek/ModelFileData/?build=" + Current.buildName + "&col=FileDataID&val=" + Current.fileDataID);
    ...
    const idiPromises = [
        fetch(API_URL + "/api/find/ItemDisplayInfo/?build=" + Current.buildName + "&col=ModelResourcesID[0]&val=" + modelFileData.values['ModelResourcesID']),
        fetch(API_URL + "/api/find/ItemDisplayInfo/?build=" + Current.buildName + "&col=ModelResourcesID[1]&val=" + modelFileData.values['ModelResourcesID'])
    ];
...
async function getFileDataIDByDisplayID(displayID){
    const cdiResponse = await fetch(API_URL + "/api/peek/CreatureDisplayInfo/?build=" + Current.buildName + "&col=ID&val=" + displayID);
    const cdiJson = await cdiResponse.json();
    
    const cmdResponse = await fetch(API_URL + "/api/peek/CreatureModelData/?build=" + Current.buildName + "&col=ID&val=" + cdiJson.values['ModelID']);
...
function loadModelTextures() {
    ...
    $.ajax({url: API_URL + "/api/texture/" + Current.fileDataID + "?build=" + Current.buildName}).done( function(data) {
        var forFDID = this.url.replace(API_URL + "/api/texture/", "").replace("?build=" + Current.buildName, "");
...
async function setModelDisplay(displayID, type){
    ...
    if (type == "creature"){
        const response = await fetch(API_URL + "/api/peek/CreatureDisplayInfo/?build=" + Current.buildName + "&col=ID&val=" + displayID);
        ...
        if (cdiRow.values['ParticleColorID'] != 0){
            const particleResponse = await fetch(API_URL + "/api/peek/ParticleColor/?build=" + Current.buildName + "&col=ID&val=" + cdiRow.values['ParticleColorID']);
        ...
        const cmdResponse = await fetch(API_URL + "/api/peek/CreatureModelData/?build=" + Current.buildName + "&col=ID&val=" + cdiRow.values['ModelID']);
        ...
        if (cmdRow.values.CreatureGeosetDataID != "0"){
            const geosetResponse = await fetch(API_URL + "/api/find/CreatureDisplayInfoGeosetData/?build=" + Current.buildName + "&col=CreatureDisplayInfoID&val=" + cdiRow.values['ID']);
...
function setModelTexture(textures, offset){
    ...
    for (let i = 0; i < textures.length; i++){
        if (offset == 11 && i == 3){
            ...
            fetch(API_URL + "/api/peek/particlecolor?build=" + Current.buildName + "&col=ID&val=" + particleColorID)
